...
ASTNodeList AOEStatement() {
  ...
  while (1) {
    ...
    else if (tokentmp != NULL && tokentmp->type == Or) {
      pList = pList->next;
      ASTNodeList child2 = relStatement();
      if (child2 != NULL) {
        node = (ASTNodeList)malloc((sizeof(ASTNode)));
...
int main(int argc, char* argv[]) {
  if (argc != 2) {
    printf("Error: usage: ./GrammarAnalysis <path_to_test-file>");
    exit(0);
  }
